...
@section('container')
    <div class="container">
        <h1 class="fw-semibold mb-3">Buat Kelas <i class="fa-solid fa-laptop-code text-primary"></i></h1>
        <form action="/dashboard/super/courses" method="post" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <label for="speaker_id" class="col-sm-2 col-form-label">Pemateri</label>
                <div class="col-sm-10">
                    <select class="form-select" name="speaker_id">
                        @foreach ($speaker as $pembicara)
                            @if (old('speaker_id') == $pembicara->id)
                                <option value="{{ $pembicara->id }}" selected>{{ $pembicara->user->name }}</option>
                            @else
                                <option value="{{ $pembicara->id }}">{{ $pembicara->user->name }}</option>
                            @endif
                        @endforeach
                    </select>
                </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control rounded-0 @error('location') is-invalid @enderror"
